...
public class PrimeNocheck {
		...
		for (int i = 2; i <= num; i++) {
			...
			if (num % i == 0) {
				if (i == num) {
					...
					/*
					 *After getting prime no. calling isPalindrom method to check whether number
					 * is palindrom or not and storing it into another arraylist
					 */
					isPalindrom(num);
					if (isPalindrom(num)) {
						arrayList2.add(num);
	...
	//5. Create a isPlaindrom method to check whether the prime no. is palindrom or not
